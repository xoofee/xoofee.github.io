---
title: "Use BaiduPCS-Go on a Headless Ubuntu Server"
date: 2026-07-13
permalink: /posts/2026/07/baidupcs_go_headless_ubuntu_server/
categories: tech
tags: [ubuntu, baidu, baidupcs-go, netdisk, server]
---

BaiduPCS-Go can run on a headless cloud Ubuntu server because it is a command-line Baidu Netdisk client.

You can either run it interactively over SSH or use one-shot commands from the normal shell. The main things to get right are:
...
For a Linux AMD64 server, download the release zip from GitHub.
...
# 2. Set a config directory

On a cloud server, I prefer setting the config directory explicitly:

```bash
export BAIDUPCS_GO_CONFIG_DIR=/home/ubuntu/.config/BaiduPCS-Go
```

If you want this to persist across SSH sessions, add it to your shell profile.
...
5. Use that cookie on the server.
...
# 4. Interactive login over SSH

You can also SSH into the server and run:
...
If Baidu asks for an image captcha, the CLI prints a local captcha path and a URL. Open the URL in your browser, read the captcha, then enter it in the SSH session.
...
```bash
ls /
download /YA2025198_Data --saveto /home/ubuntu/downloads
```

From the normal Ubuntu shell, include the binary prefix:

```bash
./BaiduPCS-Go ls /
./BaiduPCS-Go download /YA2025198_Data --saveto /home/ubuntu/downloads
```
...
If you are in the normal Ubuntu shell instead of the BaiduPCS-Go interactive shell, run:
...
For a fresh Ubuntu AMD64 server, the basic flow is:
...
The key lesson is simple: on a headless server, do the hard login verification in a browser first, then let BaiduPCS-Go use the resulting session cookie for command-line downloads.
